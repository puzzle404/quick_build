...
- **Primitives:** reuse `Qb::*` components (`Qb::BtnComponent`, `PillComponent`, `BarComponent`, `IconComponent`, `MetricCellComponent`, `SectionHeadComponent`, `TabsComponent`, `StatusDotComponent`, `AvatarComponent`, `PaginationComponent`, `FilterChipComponent`, `FileFieldComponent`). Build new shared UI as a `Qb::*` primitive rather than ad-hoc markup.
