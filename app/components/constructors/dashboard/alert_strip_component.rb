...
class Constructors::Dashboard::AlertStripComponent < ViewComponent::Base
  ...
  def build_alert_for(p)
    if p.health == :bad
      ...
      { tone: :bad, record: p, project: "#{p.code} · #{p.name}", message: "Sobrecosto proyectado +#{delta}% vs presupuesto", when: "Hoy" }
    else
      delta = (p.planned_progress - p.progress)
      { tone: :warn, record: p, project: "#{p.code} · #{p.name}", message: "Avance físico #{delta} pts por debajo del plan", when: "Hoy" }
    end
  end
end
