class Constructors::Projects::Blueprints::AiAnalysesController < Constructors::BaseController
  before_action :find_project!
  before_action :set_blueprint
  before_action :set_analysis, only: [ :show, :apply, :destroy ]

  # GET /constructors/projects/:project_id/blueprints/:blueprint_id/ai_analyses
  def index
    authorize @project, :show?

    # Load only necessary fields to avoid serialization issues with corrupted data
    @analyses = @blueprint.ai_blueprint_analyses
                          .select(:id, :blueprint_id, :status, :applied_at, :error_message, :created_at, :updated_at)
                          .recent
                          .limit(20)
  ...
  def create
    # Correr y aplicar un análisis toca las mediciones del plano: es contenido
    # de obra (editor+), no edición de los datos de la obra.
    authorize @project, :manage_content?

    filter = params[:filter] # 'muros', 'aberturas', 'pisos_losas', or nil

    # Create new analysis record
    @analysis = @blueprint.ai_blueprint_analyses.create!(status: "queued")

    # Enqueue the background job with the analysis ID
    AnalyzeBlueprintJob.perform_later(@blueprint.id, @analysis.id, filter: filter)

    respond_to do |format|
      format.html { redirect_to constructors_project_blueprint_ai_analysis_path(@project, @blueprint, @analysis), notice: "Análisis iniciado. Esto puede tomar unos segundos..." }
  ...
  def apply
    ...
    respond_to do |format|
      format.html { redirect_to constructors_project_blueprint_path(@project, @blueprint), notice: "#{applied_count} mediciones aplicadas correctamente" }
      format.json { render json: { success: true, applied_count: applied_count } }
    end
  end

  # DELETE /constructors/projects/:project_id/blueprints/:blueprint_id/ai_analyses/:id
  def destroy
    authorize @project, :manage_content?
    ...
    respond_to do |format|
      format.html { redirect_to constructors_project_blueprint_ai_analyses_path(@project, @blueprint), notice: "Análisis eliminado" }
