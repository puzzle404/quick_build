...
module Constructors
  module Projects
    class MaterialListSearchService
      ...
      def base_scope
        project.material_lists.includes(:material_list_publication, :project_stage)
      end

      def apply_query(scope)
        return scope unless query

        scope.merge(MaterialList.search_text(query))
      end

      def apply_date_filter(scope)
        Support::DateRangeFilter.apply(
          scope: scope,
          columns: %w[material_lists.updated_at material_lists.created_at],
          from: from_date,
          to: to_date
        )
      end
