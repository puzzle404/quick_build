...
#
# Searches scoped to the projects the constructor can access — owned + where
# they're a member — (projects, stages, documents, material_lists) plus their
...
module Constructors
  class SearchService
    ...
    # Todo lo que el usuario puede abrir. Si acá se colara una obra ajena, el
    # palette sería un buscador de datos de otros constructores.
    def accessible_project_ids
      @user.accessible_projects.pluck(:id)
    end
    ...
    def project_results(project_ids)
      ...
      scope = @user.accessible_projects.order(updated_at: :desc)
      if @query.present?
