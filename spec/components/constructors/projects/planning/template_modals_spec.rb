...
# Los dos modales de plantillas se montan en projects#show. El render completo
# es la red de seguridad: un `helpers.` faltante rompe la pantalla entera.
RSpec.describe "Planning template modals", type: :component do
  include ViewComponent::TestHelpers

  let(:owner) { create(:user, :constructor) }
  let(:project) { create(:project, owner: owner, start_date: Date.new(2026, 3, 1)) }

  describe Constructors::Projects::Planning::TemplateModalComponent do
  ...
  describe Constructors::Projects::Planning::SaveTemplateModalComponent do
